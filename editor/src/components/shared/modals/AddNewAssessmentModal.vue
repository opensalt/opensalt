    ...
    <div class="modal-dialog modal-xl" role="document" style="width:99%" @click.stop>
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="addNewAssessmentModalLabel"><span>{{ props.item ? 'Edit Assessment' : 'Add New Assessment' }}</span></h5>
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-primary" @click="createItem" :disabled="saving">
            ...
            <span>{{ props.item ? 'Update' : 'Create' }}</span>
          </button>
        </div>
      </div>
    </div>
...
function createItem() {
  ...
  setTimeout(() => {
    try {
      ...
      error.value = 'Failed to create assessment: ' + e.message;
    } finally {
      saving.value = false;
    }
  }, 1000);
